...
const customerRoutes: Routes = [
  { path: 'customers', component: CustomersComponent, canActivate: [AuthenticationGuard] }, //pass the guard
  { path: 'customers/new', component: NewcustomerComponent, canActivate: [AuthenticationGuard] }, //pass the guard
  { path: 'customers/:id', component: CustomerDetailComponent, canActivate: [AuthenticationGuard] } //pass the guard
];


@NgModule({
    imports: [RouterModule.forChild(customerRoutes)], // forRoot vs forChild // forChild is part of the child route //(app.module.ts) 在 Angular 裡，AppRoutingModule 是一個專門用來管理 應用程式路由 (Routing) 的模組。它的主要用途是把不同的 URL 路徑對應到不同的元件 (Component)，讓使用者在瀏覽器中切換頁面時，Angular 能顯示正確的內容。
