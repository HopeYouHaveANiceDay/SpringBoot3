...
export class InvoicesComponent implements OnInit {
  ...
  //invoiceState$: Observable<State<CustomHttpResponse<Page & User>>>;
  ...
  ngOnInit(): void {
    //this.invoiceState$ = this.customerService.searchCustomers$()//searchCustomers$ have User, we can see the user icon in the invoices page
...
整體邏輯 :
  ...
  searchCustomers(searchForm: NgForm): void {
    ...
    this.invoiceState$ = this.customerService.searchCustomers$(searchForm.value.name)
...
*/
  ...
  goToPage(pageNumber?: number): void {
    //this.invoiceState$ = this.customerService.searchCustomers$(name, pageNumber)// (name, pageNumber) ==>> take the parameters from searchCustomers$ = (name:string = '', page: number = 0) => <Observable<CustomHttpResponse<Page & User>>> in customer.service.ts
    //we can pass the page in the invoices$
